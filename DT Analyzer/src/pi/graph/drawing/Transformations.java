...
public class Transformations
{
	...
	public void transformToRealPoint(double x, double y, Point p)
	{
		...
		p.x = (int) ((x * (this.t_width / (double) this.size.width)) + content
				.getX());
		p.y = (int) ((y * (this.t_height / (double) this.size.height)) + content
				.getY());
	}
	...
	{
		int resize = 150;
		if ((p.x >= rect.x - resize) && (p.y >= rect.y - rect.height - resize)
